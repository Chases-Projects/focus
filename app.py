...
def focus():
    if request.method == "POST":
        ...
        if section == "delete":
            ...
            db.session.add(Task(content=item, category=section))
            db.session.commit()
    ...
    top3 = Task.query.filter_by(category="top3", task_date=date.today()).all()
    todo = Task.query.filter_by(category="todo", task_date=date.today()).all()
    distractions = Task.query.filter_by(category="distractions", task_date=date.today()).all()
